...
form.addEventListener('submit', function(e) {
    ...
    e.preventDefault();
    const res = checkForm();
    if (res.type === 'error') pushNotification(10, 10, 'Error', res.message, 'error');
    else {
...
table.addEventListener('dblclick', function(e) {
    ...
    if (!target) return; // якщо клікнуто не по комірці нічо не робити
    ...
    function deleteInput() {
        if (input.value === '') target.textContent = cellContent;
    ...
    document.onkeyup = function(ev) {
        if (ev.key === 'Enter') deleteInput();
    };
});
table.addEventListener('click', function(e) {
    const target = e.target.closest('th, td');
    const targetRow = e.target.closest('tr');
    if (!target && !targetRow) return undefined;
    const rows = Array.from(table.querySelectorAll('tr'));
    if (targetRow && targetRow.matches('tr')) singleActivateRow(rows, targetRow.rowIndex);
    if (target && target.matches('th') && target.dataset.type) {
        const columns = Array.from(tbody.rows).map((el)=>el.cells[target.cellIndex]);
...
function singleActivateRow(rows, index) {
    ...
    if (index !== 0 && !isActive) rows[index].classList.add('active');
    if (isActive) rows[index].classList.remove('active');
    rows.forEach((el, ind)=>{
        if (ind !== index) el.classList.remove('active');
    });
}
...
const pushNotification = (posTop, posRight, title, description, type)=>{
    ...
    if (exictingNotification) exictingNotification.remove();
...
function checkInput(firstValue, input) {
    ...
    if (firstValue[0] === '$') firstType = 'money';
    else if (!isNaN(parseFloat(firstValue))) firstType = 'number';
    else firstType = 'string';
    switch(firstType){
        case 'money':
...
function checkForm() {
    ...
    if (form.position.value.length < 4) return {
        type: 'error',
    ...
    if (form.name.value.length < 4) return {
        type: 'error',
